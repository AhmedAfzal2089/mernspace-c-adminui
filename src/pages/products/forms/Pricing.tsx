...
const Pricing = ({ selectedCategory }: PricingProps) => {
  const { data: fetchedCategory } = useQuery<Category>({
    ...
    staleTime: 1000 * 60 * 5, // enabling cacheing to do not request to server again and again
  });
  if (!fetchedCategory) {
    return null;
  }
                  ...
                  {configurationValue.availableOptions.map((option: string) => {
                    return (
                      <Col span={8} key={option}>
                        <Form.Item
                          label={option}
                          // this will generate a name , of string , then while submitting we will extract it
                          name={[
                            "priceConfiguration",
                            JSON.stringify({
